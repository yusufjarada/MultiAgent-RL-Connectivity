...
def load_results(results_dir: str) -> dict:
    """Load all result JSON files, grouped by method."""
    results = {}
...
def plot_pareto(results: dict, save_path: str):
    ...
    for method, runs in results.items():
        ...
        ax.scatter(avg_comm, avg_reward, c=color, s=200, marker='*',
                   edgecolors='white', linewidths=1.5, zorder=10, label=label)
    ...
    ax.annotate('← Better\n(less comm, more reward)', xy=(0.15, ax.get_ylim()[1] * 0.9),
...
def print_summary_table(results: dict):
    ...
    for method in ['commnet', 'ic3net', 'tarmac', 'gated_attn']:
        ...
        label = METHOD_LABELS.get(method, method)
        print(f"{label:<25} {avg_reward:>7.3f}±{std_reward:.3f} {avg_comm:>10.1%} {avg_conn:>12.4f}")
